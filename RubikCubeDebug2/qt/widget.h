...
public:
    ...
    FACE getFace(QString faceName){
        ...
        else if(faceName == "Blue") return this->back;
...
signals:
    void imageLoaded();
    void imageLoaded(QImage newImage, QGraphicsScene* scene);

    void imageAffined();
    void imageAffined(Mat& src, QGraphicsScene* scene, QImage::Format);
    void imageThreshed();
    void imageThreshed(Mat& src, QGraphicsScene* scene, QImage::Format);
...
private:
    ...
    void setHSV();
    void showHSV();
    void showImage(Mat &img, QGraphicsScene* scene, QImage::Format Format);
    void showImage(QImage Image, QGraphicsScene* scene);
...
//                        putText(face.src, it.colorNameString, ROI_rect[3*i+j].tl(), FONT_HERSHEY_PLAIN, 1, Scalar(0,255,0), 1);
        ...
        QString substring = color_this_face.chopped(9);
        if(isRotate == 0) {return substring;}
        else {return reversalStr(substring);}
    }
